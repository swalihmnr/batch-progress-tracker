...
# Remove html,body to prevent breaking the rest of the app
raw_css = re.sub(r'html,body\{[^}]+\}', '', raw_css)

# Scope generic tags to .css_lab
tags = ['ul', 'li', 'input', 'label']
for tag in tags:
    # Match tag at the start of a line or after a comma
    raw_css = re.sub(r'(^|,|\s)(' + tag + r')(?=[\s:\{,])', r'\1.css_lab \2', raw_css)

# Scope *
raw_css = raw_css.replace('*{', '.css_lab *{')

# Add scale-animation for mouth lip sync
raw_css += """
.mouth {
...
with open('src/components/chat/AvatarWhitePallet.css', 'w') as f:
    f.write(raw_css)
...
export default function AvatarCSS({ isSpeaking, userAudioLevel }) {
  ...
  useEffect(() => {
    ...
    const animateMouth = () => {
      if (mouthRef.current) {
        if (isSpeaking && userAudioLevel > 0) {
          // Calculate scale based on audio level (1.0 to 1.8)
          const newScale = 1 + (userAudioLevel / 100) * 0.8;
          mouthRef.current.style.transform = `scaleY(${newScale}) translateY(${userAudioLevel / 20}px)`;
        } else {
          mouthRef.current.style.transform = `scaleY(1) translateY(0px)`;
        }
      }
      setTimeout(() => {
  ...
  }, [isSpeaking, userAudioLevel]);

  return (
    <div className="w-full h-full flex items-center justify-center bg-[#f8f8ff] overflow-hidden relative" style={{ transform: 'scale(0.8)' }}>
        <div className="css_lab">
            <input type="radio" name="hair" id="style1" defaultChecked />
            ...
            <ul>
                <li><label htmlFor="style1">1</label></li>
                <li><label htmlFor="style2">2</label></li>
                <li><label htmlFor="style3">3</label></li>
                <li><label htmlFor="style4">4</label></li>
                <li><label htmlFor="style5">5</label></li>
            </ul>
            <div className="head">
                <div className="neck">
                    <p></p><p></p>
...
with open('src/components/chat/AvatarCSS.jsx', 'w') as f:
    f.write(react_code)
